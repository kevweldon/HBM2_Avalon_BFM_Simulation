...
if { [get_collection_size [get_ports -nowarn {altera_reserved_tck}]] > 0 } {
   ...
   if {[string equal quartus_sta $::TimeQuestInfo(nameofexecutable)]} {
      post_message -type warning "The HBM2 IP Example Design is using default JTAG timing constraints from jtag_example.sdc. For correct hardware behavior, you must review the timing constraints and ensure that they accurately reflect your JTAG topology and clock speed."
   }
}

set ports [get_ports *]
set num_in_port_list  0
foreach_in_collection port $ports {
  set port_name [get_port_info -name $port]
  set result [regexp {asic_tb} $port_name match]
  if {$result==1} {
    incr num_in_port_list
  }
}

if { $num_in_port_list>0 } {
  set_false_path -from [get_clocks {phylite|phylite_usr_clk}] \
     -to [get_clocks {dut_example_design|dut_example_design|uib_hr_core_clk}]
  set_false_path -from [get_clocks {dut_example_design|dut_example_design|uib_hr_core_clk}] \
     -to [get_clocks {phylite|phylite_usr_clk}]
     
  set_multicycle_path -from [get_clocks {phylite|phylite_wf_clk_grp_1_0 phylite|phylite_wf_clk_grp_1_0_neg}] -to [get_clocks {phylite_group_1_io_interface_conduit_end_io_strobe_out}] -setup -end 5
  set_multicycle_path -from [get_clocks {phylite|phylite_wf_clk_grp_1_0 phylite|phylite_wf_clk_grp_1_0_neg}] -to [get_clocks {phylite_group_1_io_interface_conduit_end_io_strobe_out}] -hold -end 5

  create_clock -name core_refclk -period 10.000 [get_pins {phylite|phylite|core|arch_inst|pll_inst|pll_inst|core_refclk}]
