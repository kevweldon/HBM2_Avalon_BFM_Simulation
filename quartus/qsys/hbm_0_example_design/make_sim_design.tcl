...
cd $ex_design_path
exec -ignorestderr $ip_generate_exe_path --pro --simulation=${lang} --part=$device $qsys_file >>& ip_generate.out

post_message "Finalizing..."

set sim_scripts [list]
set vcs_script "${ex_design_path}/${system_name}/sim/synopsys/vcs/vcs_setup.sh"
set xcelium_script "${ex_design_path}/${system_name}/sim/xcelium/xcelium_setup.sh"

if {$lang == "VERILOG"} {
   lappend sim_scripts $vcs_script
}
lappend sim_scripts "${ex_design_path}/${system_name}/sim/synopsys/vcsmx/vcsmx_setup.sh"
lappend sim_scripts "${ex_design_path}/${system_name}/sim/cadence/ncsim_setup.sh"
